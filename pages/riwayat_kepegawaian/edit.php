...
// Get riwayat data
$stmt = $pdo->prepare("SELECT * FROM riwayat_kepegawaian WHERE id = ?");
...
$dokumenInfo = null;
if ($riwayat['dokumen_sk_id']) {
    $stmt_dok = $pdo->prepare("SELECT * FROM dokumen_sk WHERE id = ?");
    $stmt_dok->execute([$riwayat['dokumen_sk_id']]);
    $dokumenInfo = $stmt_dok->fetch();

    // Get all pegawai associated with this document
    $stmt_assoc = $pdo->prepare("SELECT pegawai_id FROM dokumen_sk_pegawai WHERE dokumen_sk_id = ?");
    $stmt_assoc->execute([$riwayat['dokumen_sk_id']]);
    $associatedPegawai = $stmt_assoc->fetchAll(PDO::FETCH_COLUMN);
} else {
    $associatedPegawai = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dokumen_sk_id = $riwayat['dokumen_sk_id']; // Preserve existing document if not updated
    ...
    if (empty($jenis_perubahan) || empty($tanggal_efektif) || empty($pegawai_ids)) {
        ...
        // Handle document upload if provided and valid
        if (isset($_FILES['dokumen_sk']) && $_FILES['dokumen_sk']['error'] === UPLOAD_ERR_OK && $_FILES['dokumen_sk']['size'] > 0) {
            // Delete old document if exists
            if ($riwayat['dokumen_sk_id']) {
                $oldDocInfo = $pdo->query("SELECT dokumen_sk FROM dokumen_sk WHERE id = {$riwayat['dokumen_sk_id']} LIMIT 1")->fetch();
                if ($oldDocInfo && !empty($oldDocInfo['dokumen_sk'])) {
                    deleteDocument($oldDocInfo['dokumen_sk'], 'dokumen_sk');
                }
                // Delete the reference from dokumen_sk table
                $pdo->prepare("DELETE FROM dokumen_sk WHERE id = ?")->execute([$riwayat['dokumen_sk_id']]);
            }

            // Upload new document
            $uploadedDoc = uploadDocument($_FILES['dokumen_sk'], 0, 'dokumen_sk');
            if ($uploadedDoc) {
                // Create new entry in dokumen_sk table
            ...
            if ($riwayat['dokumen_sk_id']) {
                ...
                $stmt_update = $pdo->prepare("
                    UPDATE dokumen_sk
                    SET nomor_sk = COALESCE(NULLIF(?, ''), nomor_sk),
                        judul = COALESCE(NULLIF(?, ''), judul),
                        tanggal_sk = COALESCE(NULLIF(?, ''), tanggal_sk),
                        jenis_perubahan = COALESCE(NULLIF(?, ''), jenis_perubahan)
                    WHERE id = ?
                ");

                $stmt_update->execute([$nomor_sk, $judul_sk, $tanggal_sk, $jenis_perubahan, $riwayat['dokumen_sk_id']]);
            } else {
        ...
        if (empty($error)) {
            ...
            try {
                // Delete old relationships for this dokumen_sk_id in dokumen_sk_pegawai table
                $stmt_del_rel = $pdo->prepare("DELETE FROM dokumen_sk_pegawai WHERE dokumen_sk_id = ?");
                $stmt_del_rel->execute([$riwayat['dokumen_sk_id']]);

                // Delete old riwayat_kepegawaian records for this dokumen_sk_id
                $stmt_del_rk = $pdo->prepare("DELETE FROM riwayat_kepegawaian WHERE dokumen_sk_id = ?");
                $stmt_del_rk->execute([$riwayat['dokumen_sk_id']]);
                ...
                foreach ($pegawai_ids as $pegawai_id) {
                    ...
                    // Insert new riwayat_kepegawaian into database
                    $stmt_insert_rk = $pdo->prepare("
                        INSERT INTO riwayat_kepegawaian (pegawai_id, jenis_perubahan, keterangan, tanggal_efektif, dokumen_sk_id)
                        VALUES (?, ?, ?, ?, ?)
                    ");

                    if ($stmt_insert_rk->execute([$pegawai_id, $jenis_perubahan, $keterangan, $tanggal_efektif, $dokumen_sk_id])) {
                        // Also insert into dokumen_sk_pegawai table to maintain many-to-many relationship
                        $stmt_insert_rel = $pdo->prepare("
                            INSERT INTO dokumen_sk_pegawai (dokumen_sk_id, pegawai_id)
                            VALUES (?, ?)
                        ");
                        $stmt_insert_rel->execute([$dokumen_sk_id, $pegawai_id]);
                        $successCount++;
                    } else {
                ...
                logActivity($_SESSION['user_id'], 'update_riwayat_kepegawaian', "Updated riwayat_kepegawaian for $successCount pegawai");
                setAlert('success', "$successCount Riwayat kepegawaian berhasil diperbarui!");
                redirect('index.php?page=riwayat_kepegawaian');
            } catch (Exception $e) {
...
<div class="mb-6">
    <div class="flex items-center space-x-4 mb-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Edit Riwayat Kepegawaian</h1>
            <p class="text-gray-500 mt-1">Perbarui informasi riwayat perubahan kepegawaian</p>
        </div>
    </div>
</div>
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="pegawai_ids[]" multiple size="5" required
                    ...
                    <?php foreach ($pegawaiList as $pegawai): ?>
                        <option value="<?php echo $pegawai['id']; ?>"
                                <?php echo (isset($_POST['pegawai_ids']) && in_array($pegawai['id'], $_POST['pegawai_ids'])) || (!isset($_POST['pegawai_ids']) && in_array($pegawai['id'], $associatedPegawai)) ? 'selected' : ''; ?>>
                ...
                <select name="jenis_perubahan" required
                    ...
                    <?php foreach ($jenis as $j): ?>
                        <option value="<?php echo $j['jenis_perubahan']; ?>"
                                <?php echo (isset($_POST['jenis_perubahan']) ? $_POST['jenis_perubahan'] : $riwayat['jenis_perubahan']) === $j['jenis_perubahan'] ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <input type="date" name="tanggal_efektif" required
                       value="<?php echo isset($_POST['tanggal_efektif']) ? htmlspecialchars($_POST['tanggal_efektif']) : (isset($riwayat['tanggal_efektif']) ? htmlspecialchars($riwayat['tanggal_efektif']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
            ...
            <div>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl">
                    <div class="space-y-1 text-center">
                        ...
                        <p class="text-xs text-gray-500">Kosongkan jika tidak ingin mengganti dokumen</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div>
            ...
            <textarea name="keterangan" rows="4"
                      class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"><?php echo isset($_POST['keterangan']) ? htmlspecialchars($_POST['keterangan']) : (isset($riwayat['keterangan']) ? htmlspecialchars($riwayat['keterangan']) : ''); ?></textarea>
        </div>
