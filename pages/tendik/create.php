...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pegawai_id) || empty($unit_kerja_id) || empty($jabatan) || empty($status_kepegawaian) || empty($tanggal_mulai_kerja)) {
        ...
        // Check if pegawai_id already exists in tendik table
        $stmt = $pdo->prepare("SELECT id FROM tendik WHERE pegawai_id = ?");
        $stmt->execute([$pegawai_id]);

        if ($stmt->fetch()) {
            $error = 'Pegawai ini sudah terdaftar sebagai tenaga kependidikan!';
        } else {
            // Insert tendik into database
            $stmt = $pdo->prepare("
                INSERT INTO tendik (pegawai_id, nip, unit_kerja_id, jabatan, status_kepegawaian, golongan, tanggal_mulai_kerja, tanggal_selesai)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");

            if ($stmt->execute([$pegawai_id, $nip, $unit_kerja_id, $jabatan, $status_kepegawaian, $golongan, $tanggal_mulai_kerja, $tanggal_selesai])) {
                logActivity($_SESSION['user_id'], 'create_tendik', "Created new tendik for pegawai ID: $pegawai_id");
                setAlert('success', 'Tenaga kependidikan berhasil ditambahkan!');
                redirect('index.php?page=tendik');
            } else {
                $error = 'Gagal menambahkan tenaga kependidikan!';
            }
        }
    }
}
...
<div class="mb-6">
    <div class="flex items-center space-x-4 mb-4">
        <a href="index.php?page=tendik" class="text-gray-600 hover:text-gray-800">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Tambah Tendik Baru</h1>
            <p class="text-gray-500 mt-1">Tambahkan data tenaga kependidikan baru</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm p-6">
    <form method="POST" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="unit_kerja_id" required
                    ...
                    foreach ($unitKerjaList as $unit): ?>
                        <option value="<?php echo $unit['id']; ?>"
                                <?php echo (isset($_POST['unit_kerja_id']) && $_POST['unit_kerja_id'] == $unit['id']) ? 'selected' : ''; ?>>
                ...
                <select name="status_kepegawaian" required
                    ...
                    <?php foreach ($status as $s): ?>
                        <option value="<?php echo $s['status_kepegawaian']; ?>"
                                <?php echo (isset($_POST['status_kepegawaian']) && $_POST['status_kepegawaian'] === $s['status_kepegawaian']) ? 'selected' : ''; ?>>
                    ...
                    <option value="honorer" <?php echo (isset($_POST['status_kepegawaian']) && $_POST['status_kepegawaian'] === 'honorer') ? 'selected' : ''; ?>>Honorer</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="golongan"
                       value="<?php echo isset($_POST['golongan']) ? htmlspecialchars($_POST['golongan']) : ''; ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            ...
            <div>
                ...
                <input type="date" name="tanggal_mulai_kerja" required
                       value="<?php echo isset($_POST['tanggal_mulai_kerja']) ? htmlspecialchars($_POST['tanggal_mulai_kerja']) : ''; ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
        ...
        <div class="flex space-x-3 pt-4 border-t">
            ...
            <a href="index.php?page=tendik" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2 px-6 rounded-xl transition-colors inline-block">
                Batal
            </a>
        </div>
    </form>
