...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($dosen_id) || empty($prodi_id) || empty($status_hubungan) || empty($tanggal_mulai)) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM dosen_prodi WHERE dosen_id = ? AND prodi_id = ? AND status_hubungan = ?");
        $stmt->execute([$dosen_id, $prodi_id, $status_hubungan]);

        if ($stmt->fetch()) {
            $error = 'Hubungan dosen dan prodi dengan status ini sudah terdaftar!';
        } else {
            // Insert dosen_prodi into database
            $stmt = $pdo->prepare("
                INSERT INTO dosen_prodi (dosen_id, prodi_id, status_hubungan, is_kaprodi, tanggal_mulai, tanggal_selesai)
                VALUES (?, ?, ?, ?, ?, ?)
            ");

            if ($stmt->execute([$dosen_id, $prodi_id, $status_hubungan, $is_kaprodi, $tanggal_mulai, $tanggal_selesai])) {
                logActivity($_SESSION['user_id'], 'create_dosen_prodi', "Created new dosen_prodi: dosen ID $dosen_id, prodi ID $prodi_id");
                setAlert('success', 'Hubungan dosen dan prodi berhasil ditambahkan!');
                redirect('index.php?page=dosen_prodi');
            } else {
                $error = 'Gagal menambahkan hubungan dosen dan prodi!';
            }
        }
    }
}
...
<div class="mb-6">
    <div class="flex items-center space-x-4 mb-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Tambah Hubungan Dosen-Prodi Baru</h1>
            <p class="text-gray-500 mt-1">Tambahkan data hubungan dosen dan program studi</p>
        </div>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="dosen_id" required
                    ...
                    <?php foreach ($dosenList as $dosen): ?>
                        <option value="<?php echo $dosen['id']; ?>"
                                <?php echo (isset($_POST['dosen_id']) && $_POST['dosen_id'] == $dosen['id']) ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <select name="status_hubungan" required
                    ...
                    <option value="homebase" <?php echo (isset($_POST['status_hubungan']) && $_POST['status_hubungan'] === 'homebase') ? 'selected' : ''; ?>>Homebase</option>
                    <option value="pengampu" <?php echo (isset($_POST['status_hubungan']) && $_POST['status_hubungan'] === 'pengampu') ? 'selected' : ''; ?>>Pengampu</option>
                    <option value="tamu" <?php echo (isset($_POST['status_hubungan']) && $_POST['status_hubungan'] === 'tamu') ? 'selected' : ''; ?>>Tamu</option>
                </select>
            </div>
        ...
        <div>
            ...
            <div class="flex items-center space-x-3 mt-3">
                <input type="checkbox" name="is_kaprodi" id="is_kaprodi" value="1"
                       <?php echo (isset($_POST['is_kaprodi']) && $_POST['is_kaprodi']) ? 'checked' : ''; ?>
