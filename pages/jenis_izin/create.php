...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_izin)) {
        ...
        // Check if jenis izin with same name already exists
        $stmt = $pdo->prepare("SELECT id_jenis_izin FROM jenis_izin WHERE nama_izin = ?");
        $stmt->execute([$nama_izin]);
        $existing = $stmt->fetch();

        if ($existing) {
            $error = "Jenis izin dengan nama tersebut sudah ada.";
        } else {
            // Insert new jenis izin
            $stmt = $pdo->prepare("INSERT INTO jenis_izin (nama_izin, keterangan, is_potong_cuti) VALUES (?, ?, ?)");
            $result = $stmt->execute([$nama_izin, $keterangan, $is_potong_cuti]);

            if ($result) {
                $_SESSION['success_message'] = "Jenis izin berhasil ditambahkan.";
...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mt-2">Tambah Jenis Izin Baru</h1>
</div>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="nama_izin" id="nama_izin" value="<?php echo isset($_POST['nama_izin']) ? htmlspecialchars($_POST['nama_izin']) : ''; ?>"
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_potong_cuti" id="is_potong_cuti" value="1"
                       ...
                       <?php echo isset($_POST['is_potong_cuti']) ? 'checked' : ''; ?>>
