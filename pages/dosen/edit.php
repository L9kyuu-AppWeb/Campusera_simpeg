...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pegawai_id) || empty($status_dosen) || empty($status_ikatan) || empty($jenjang_pendidikan)) {
        ...
        // Check if pegawai_id already exists in dosen table for other records
        $stmt = $pdo->prepare("SELECT id FROM dosen WHERE pegawai_id = ? AND id != ?");
        $stmt->execute([$pegawai_id, $dosenId]);

        if ($stmt->fetch()) {
            $error = 'Pegawai ini sudah terdaftar sebagai dosen!';
        } else {
            // Update dosen in database
            $stmt = $pdo->prepare("
                UPDATE dosen
                SET pegawai_id = ?, nidn = ?, nidk = ?, prodi_homebase_id = ?, status_dosen = ?, status_ikatan = ?,
            ...
            if ($stmt->execute([
                ...
                logActivity($_SESSION['user_id'], 'update_dosen', "Updated dosen ID: $dosenId");
                setAlert('success', 'Dosen berhasil diperbarui!');
                redirect('index.php?page=dosen');
            } else {
...
<div class="mb-6">
    <div class="flex items-center space-x-4 mb-4">
        ...
        <div>
            ...
            <p class="text-gray-500 mt-1">Perbarui informasi dosen</p>
        </div>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="pegawai_id" required
                    ...
                    <?php foreach ($pegawaiList as $pegawai): ?>
                        <option value="<?php echo $pegawai['id']; ?>"
                                <?php echo (isset($_POST['pegawai_id']) ? $_POST['pegawai_id'] : $dosen['pegawai_id']) == $pegawai['id'] ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <input type="text" name="nidn"
                       value="<?php echo isset($_POST['nidn']) ? htmlspecialchars($_POST['nidn']) : (isset($dosen['nidn']) ? htmlspecialchars($dosen['nidn']) : ''); ?>"
            ...
            <div>
                ...
                <input type="text" name="nidk"
                       value="<?php echo isset($_POST['nidk']) ? htmlspecialchars($_POST['nidk']) : (isset($dosen['nidk']) ? htmlspecialchars($dosen['nidk']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
                ...
                <select name="prodi_homebase_id"
                    ...
                    foreach ($prodiList as $prodi): ?>
                        <option value="<?php echo $prodi['id']; ?>"
                                <?php echo (isset($_POST['prodi_homebase_id']) ? $_POST['prodi_homebase_id'] : $dosen['prodi_homebase_id']) == $prodi['id'] ? 'selected' : ''; ?>>
                ...
                <select name="status_dosen" required
                    ...
                    <?php foreach ($status as $s): ?>
                        <option value="<?php echo $s['status_dosen']; ?>"
                                <?php echo (isset($_POST['status_dosen']) ? $_POST['status_dosen'] : $dosen['status_dosen']) === $s['status_dosen'] ? 'selected' : ''; ?>>
                            <?php echo ucfirst(str_replace('_', ' ', $s['status_dosen'])); ?>
                        </option>
                    <?php endforeach; ?>
                    <!-- Default options -->
                    <option value="tetap_yayasan" <?php echo (isset($_POST['status_dosen']) ? $_POST['status_dosen'] : (isset($dosen['status_dosen']) ? $dosen['status_dosen'] : '')) === 'tetap_yayasan' ? 'selected' : ''; ?>>Tetap Yayasan</option>
                    <option value="tetap_dikti" <?php echo (isset($_POST['status_dosen']) ? $_POST['status_dosen'] : (isset($dosen['status_dosen']) ? $dosen['status_dosen'] : '')) === 'tetap_dikti' ? 'selected' : ''; ?>>Tetap Dikti</option>
                    <option value="luar_biasa" <?php echo (isset($_POST['status_dosen']) ? $_POST['status_dosen'] : (isset($dosen['status_dosen']) ? $dosen['status_dosen'] : '')) === 'luar_biasa' ? 'selected' : ''; ?>>Luar Biasa</option>
                    <option value="honorer" <?php echo (isset($_POST['status_dosen']) ? $_POST['status_dosen'] : (isset($dosen['status_dosen']) ? $dosen['status_dosen'] : '')) === 'honorer' ? 'selected' : ''; ?>>Honorer</option>
                </select>
            </div>
                ...
                <select name="status_ikatan" required
                    ...
                    <?php foreach ($ikatan as $i): ?>
                        <option value="<?php echo $i['status_ikatan']; ?>"
                                <?php echo (isset($_POST['status_ikatan']) ? $_POST['status_ikatan'] : $dosen['status_ikatan']) === $i['status_ikatan'] ? 'selected' : ''; ?>>
                            <?php echo ucfirst(str_replace('_', ' ', $i['status_ikatan'])); ?>
                        </option>
                    <?php endforeach; ?>
                    <!-- Default options -->
                    <option value="tetap" <?php echo ((isset($_POST['status_ikatan']) ? $_POST['status_ikatan'] : (isset($dosen['status_ikatan']) ? $dosen['status_ikatan'] : ''))) === 'tetap' ? 'selected' : ''; ?>>Tetap</option>
                    <option value="kontrak" <?php echo ((isset($_POST['status_ikatan']) ? $_POST['status_ikatan'] : (isset($dosen['status_ikatan']) ? $dosen['status_ikatan'] : ''))) === 'kontrak' ? 'selected' : ''; ?>>Kontrak</option>
                    <option value="part_time" <?php echo ((isset($_POST['status_ikatan']) ? $_POST['status_ikatan'] : (isset($dosen['status_ikatan']) ? $dosen['status_ikatan'] : ''))) === 'part_time' ? 'selected' : ''; ?>>Part Time</option>
                </select>
            </div>
                ...
                <select name="jenjang_pendidikan" required
                    ...
                    <?php foreach ($jenjang as $j): ?>
                        <option value="<?php echo $j['jenjang_pendidikan']; ?>"
                                <?php echo (isset($_POST['jenjang_pendidikan']) ? $_POST['jenjang_pendidikan'] : $dosen['jenjang_pendidikan']) === $j['jenjang_pendidikan'] ? 'selected' : ''; ?>>
                            <?php echo $j['jenjang_pendidikan']; ?>
                        </option>
                    <?php endforeach; ?>
                    <!-- Default options -->
                    <option value="S1" <?php echo ((isset($_POST['jenjang_pendidikan']) ? $_POST['jenjang_pendidikan'] : (isset($dosen['jenjang_pendidikan']) ? $dosen['jenjang_pendidikan'] : ''))) === 'S1' ? 'selected' : ''; ?>>S1</option>
                    <option value="S2" <?php echo ((isset($_POST['jenjang_pendidikan']) ? $_POST['jenjang_pendidikan'] : (isset($dosen['jenjang_pendidikan']) ? $dosen['jenjang_pendidikan'] : ''))) === 'S2' ? 'selected' : ''; ?>>S2</option>
                    <option value="S3" <?php echo ((isset($_POST['jenjang_pendidikan']) ? $_POST['jenjang_pendidikan'] : (isset($dosen['jenjang_pendidikan']) ? $dosen['jenjang_pendidikan'] : ''))) === 'S3' ? 'selected' : ''; ?>>S3</option>
                </select>
            </div>
                ...
                <select name="jabatan_fungsional"
                    ...
                    <?php foreach ($jabatan as $j): ?>
                        <option value="<?php echo $j['jabatan_fungsional']; ?>"
                                <?php echo (isset($_POST['jabatan_fungsional']) ? $_POST['jabatan_fungsional'] : $dosen['jabatan_fungsional']) === $j['jabatan_fungsional'] ? 'selected' : ''; ?>>
                            <?php echo ucfirst(str_replace('_', ' ', $j['jabatan_fungsional'])); ?>
                        </option>
                    <?php endforeach; ?>
                    <!-- Default options -->
                    <option value="asisten_ahli" <?php echo ((isset($_POST['jabatan_fungsional']) ? $_POST['jabatan_fungsional'] : (isset($dosen['jabatan_fungsional']) ? $dosen['jabatan_fungsional'] : ''))) === 'asisten_ahli' ? 'selected' : ''; ?>>Asisten Ahli</option>
                    <option value="lektor" <?php echo ((isset($_POST['jabatan_fungsional']) ? $_POST['jabatan_fungsional'] : (isset($dosen['jabatan_fungsional']) ? $dosen['jabatan_fungsional'] : ''))) === 'lektor' ? 'selected' : ''; ?>>Lektor</option>
                    <option value="lektor_kepala" <?php echo ((isset($_POST['jabatan_fungsional']) ? $_POST['jabatan_fungsional'] : (isset($dosen['jabatan_fungsional']) ? $dosen['jabatan_fungsional'] : ''))) === 'lektor_kepala' ? 'selected' : ''; ?>>Lektor Kepala</option>
                    <option value="guru_besar" <?php echo ((isset($_POST['jabatan_fungsional']) ? $_POST['jabatan_fungsional'] : (isset($dosen['jabatan_fungsional']) ? $dosen['jabatan_fungsional'] : ''))) === 'guru_besar' ? 'selected' : ''; ?>>Guru Besar</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_mulai_mengajar"
                       value="<?php echo isset($_POST['tanggal_mulai_mengajar']) ? htmlspecialchars($_POST['tanggal_mulai_mengajar']) : (isset($dosen['tanggal_mulai_mengajar']) ? htmlspecialchars($dosen['tanggal_mulai_mengajar']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_selesai"
                       value="<?php echo isset($_POST['tanggal_selesai']) ? htmlspecialchars($_POST['tanggal_selesai']) : (isset($dosen['tanggal_selesai']) ? htmlspecialchars($dosen['tanggal_selesai']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            ...
            <div>
                ...
                <input type="text" name="no_sertifikat_pendidik"
                       value="<?php echo isset($_POST['no_sertifikat_pendidik']) ? htmlspecialchars($_POST['no_sertifikat_pendidik']) : (isset($dosen['no_sertifikat_pendidik']) ? htmlspecialchars($dosen['no_sertifikat_pendidik']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
            ...
            <div>
                ...
                <input type="text" name="bidang_keahlian"
                       value="<?php echo isset($_POST['bidang_keahlian']) ? htmlspecialchars($_POST['bidang_keahlian']) : (isset($dosen['bidang_keahlian']) ? htmlspecialchars($dosen['bidang_keahlian']) : ''); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="flex items-center space-x-3 mt-3">
                <input type="checkbox" name="sertifikat_pendidik" id="sertifikat_pendidik" value="1"
                       <?php echo (isset($_POST['sertifikat_pendidik']) ? $_POST['sertifikat_pendidik'] : (isset($dosen['sertifikat_pendidik']) ? $dosen['sertifikat_pendidik'] : 0)) ? 'checked' : ''; ?>
