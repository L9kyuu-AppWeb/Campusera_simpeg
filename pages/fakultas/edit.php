...
// Get fakultas data
$stmt = $pdo->prepare("SELECT * FROM fakultas WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($kode_fakultas) || empty($nama_fakultas)) {
        ...
        // Check if kode_fakultas already exists for other records
        $stmt = $pdo->prepare("SELECT id FROM fakultas WHERE kode_fakultas = ? AND id != ?");
        $stmt->execute([$kode_fakultas, $fakultasId]);

        if ($stmt->fetch()) {
            $error = 'Kode fakultas sudah digunakan!';
        } else {
            // Update fakultas in database
            $stmt = $pdo->prepare("
                UPDATE fakultas
                SET kode_fakultas = ?, nama_fakultas = ?, dekan_id = ?
                WHERE id = ?
            ");

            if ($stmt->execute([$kode_fakultas, $nama_fakultas, $dekan_id, $fakultasId])) {
                logActivity($_SESSION['user_id'], 'update_fakultas', "Updated fakultas: $nama_fakultas");
                setAlert('success', 'Fakultas berhasil diperbarui!');
                redirect('index.php?page=fakultas');
            } else {
                $error = 'Gagal memperbarui fakultas!';
            }
        }
    }
}
...
<div class="mb-6">
    <div class="flex items-center space-x-4 mb-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Edit Fakultas</h1>
            <p class="text-gray-500 mt-1">Perbarui informasi fakultas</p>
        </div>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="kode_fakultas" required
                       value="<?php echo isset($_POST['kode_fakultas']) ? htmlspecialchars($_POST['kode_fakultas']) : htmlspecialchars($fakultas['kode_fakultas']); ?>"
            ...
            <div>
                ...
                <input type="text" name="nama_fakultas" required
                       value="<?php echo isset($_POST['nama_fakultas']) ? htmlspecialchars($_POST['nama_fakultas']) : htmlspecialchars($fakultas['nama_fakultas']); ?>"
                       class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
            </div>
                ...
                <select name="dekan_id"
                    ...
                    <?php foreach ($dosenList as $dosen): ?>
                        <option value="<?php echo $dosen['id']; ?>"
                                <?php echo (isset($_POST['dekan_id']) ? $_POST['dekan_id'] : $fakultas['dekan_id']) == $dosen['id'] ? 'selected' : ''; ?>>
